...
$(function (a) {
    a(".diagram-img-big").magnificPopup({
        ...
        "image": {
            "tError": "该图未能正常读取...",
            "titleSrc": function (b) {
                return b.el.attr('title') + '<small>商业盗图要遭剁手-优客逸家</small>';
            }
        }
        ...
        "image": {
            ...
            "tError": "暂时没找到该户型图！",
            "titleSrc": function (b) {
                return "<small>商业盗图要遭剁手-优客逸家</small>";
            }
        }
    });
}), $(function (a) {
    ...
    f.on("click", function () {
        $(this).hasClass("switch_1") ? (e.stop(true,true).animate({
            "width": "+150px"
        }, "100"), $(this).removeClass("switch_1")) : (e.stop(true,true).animate({
            "width": "+250px"
...
$(function () {
    ...
    $(".ImageMapDiv").click(function () {
        ...
        t_2 == 1 && $.getScript("/Public/user/js/validation.min.js", function () {
            var select = $(".room_select");
            select.validation();
            var sub = $(".btn-valid");
            sub.on("click", function () {
                if (select.valid() == 0) return !1;
                var name = $(".pro_message_name").val(),house_id = $(".form-house-id").val() ,area_id = $(".form-area-id").val(),tel = $(".pro_message_tel").val(), sex = $("input[name='sex']:checked").val(), people = $("input[name='people']:checked").val(), remarks = $(".pro_message_remarks").val(), userorigin = $(".ddl_UserOrigin option:selected").val(), proData = "name=" + name;
                proData += "&tel=" + tel, proData += "&house_id=" + house_id, proData += "&area_id=" + area_id, proData += "&sex=" + sex, proData += "&people=" + people, proData += "&remarks=" + remarks, proData += "&userorigin=" + userorigin, proData += "&cityId=" + $(".hiddencityId").val();
                ...
                $.each(xz, function (index, file) {
                    ...
                    "type": "POST",
                    "url": "/index/rent/ajaxRent",
                    "data": proData,
                    "dataType": "html",
                    "success": function (view) {
                        $("#lookAtHouseSuccessContainer").html(view);
                        $(".select_call_name").text(name);
